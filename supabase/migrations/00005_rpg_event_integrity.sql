-- DAYVERN Migration 00005: RPG Event State Integrity & Hardened RPC
-- Replaces process_rpg_progression_tx to enforce strict database state verification for Habits, Quests, and Rewards
...
BEGIN
    ...
    ELSIF p_event_type = 'habit_completed' THEN
        -- Target ID required: NO fallback rewards allowed
        IF p_target_id IS NULL THEN
            RAISE EXCEPTION 'Habit target ID is required.';
        END IF;

        -- Row lock target habit and verify ownership
        ...
        -- Check if already completed today in habit_logs
        SELECT EXISTS (
        ...
        -- Insert habit log atomically inside transaction
    ...
    ELSIF p_event_type = 'quest_completed' THEN
        ...
        -- Row lock target quest and verify ownership
        ...
        -- Update quest status atomically
    ...
    ELSIF p_event_type = 'reward_claimed' THEN
        ...
        -- Row lock target reward and verify ownership
        ...
        -- Update reward claimed status atomically
